...
import os
import argparse
from .compiler import XivCompiler, CompilerError # クラスとしてインポート
from argparse import Namespace # For args type hint
...
def main() -> None:
    ...
    main_xiv_file: str = args.input_file
    templates_dir: str = args.templates_dir
    output_file: str = args.output_file

    # コンパイラのインスタンスを作成
    compiler: XivCompiler = XivCompiler()
    ...
    try:
        compiled_html_output: str = compiler.compile(main_xiv_file, templates_dir)
    ...
    except (FileNotFoundError, ValueError, CompilerError) as e:
        print(f"\n❌ コンパイル中にエラーが発生しました: {e}")
